...
- **소셜 채널 아이콘** — 헤더의 공유 버튼(카카오톡·페이스북·트위터·링크드인)을
  아이원소프트뱅크가 운영하는 채널 5개 바로가기로 바꿨습니다. 순서는 네이버 카페 ·
  네이버 블로그 · 인스타그램 · 카카오톡 채널 · 유튜브입니다. 기존 디자인(회색
  라운드 사각형에 흰 글리프, 마우스를 올리면 브랜드 컬러)을 그대로 따르도록
  `common/img/sns-ione.svg` 스프라이트를 새로 만들었습니다. 모바일은 헤더 폭이
  좁아 아이콘을 18px로 줄여 로고와 겹치지 않게 했습니다.
